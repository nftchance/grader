...
import { Plus, Shuffle, Trash } from "lucide-react";
...
const Tool = () => {
    ...
    const colorMath = useMemo(
        () => new ColorMath(colorMode, 0, 0),
        [colorMode]
    );
    ...
    const [points, setPoints] = useState(
        DEFAULT_GRADIENT.length * POINTS_SCALE_FACTOR
    );
    ...
    const hasMadeChange = useMemo(
        () =>
            !colors.every((color, idx) => {
                const defaultColor = DEFAULT_GRADIENT[idx];
                return (
                    color.color === defaultColor.color &&
                    color.domain === defaultColor.domain &&
                    color.locked === defaultColor.locked
                );
            }),
    ...
    const usingDefaultScale = useMemo(() => {
        return trimmedColors.every((color, idx) => {
            return (
                color.domain ===
                colorMath.indexToDomainPos(trimmedColors, idx + 1)
            );
        });
    }, [colorMath, trimmedColors]);

    const colorSquishedDomain = useMemo(
        () => (1 + colors[colors.length - 2].domain) / 2,
        [colors]
    );

    // Building the color scale with the proper domain
    const joiningColors = useMemo(
        () =>
    ...
    const colorAddedColors = useMemo(
        () => [...[colors[0]], ...joiningColors, colors[colors.length - 1]],
        [colors, joiningColors]
    );

    const URLHead = `${window.location.href.split("?")[0]}`;

    const URLTail = fixedEncodeURIComponent(
        `cm=${colorMode}&gcm=${gradientColorMode}&cs=${colors
            .map((color) => color.color)
            .join("&cs=")}&ds=${colors
            .map((color) => color.domain)
            .join("&ds=")}&d=${degree}&p=${points}&g=${code.replace(
            "background: ",
            ""
        )}&f=${Math.random() > 0.5 ? true : false}&s=${score}`
    );

    const chromaSaveURL = `${URLHead}?` + URLTail;

    const shareMessage = `I just used @trycolorspace and made this${
        score > 81 ? " perfect " : " "
    }palette - what's your score?%0A%0A${chromaSaveURL}`;
    ...
    if (hasMadeChange) handleURLUpdate();
    ...
    useEffect(() => {
        ...
        const handleQueryParams = () => {
            ...
            if (
                queryParams.getAll("cs").length !== 0 &&
                queryParams.getAll("ds").length !== 0
            ) {
        ...
        if (queryParams.getAll("cs").length !== 0) handleQueryParams();
    }, [colorMath]);
    ...
    useEffect(() => {
        ...
        setScore(colorMath.chromaGradientScore(chromaStringGradient(colors)));
    }, [colorMath, gradientColorMode, points, degree, colors, chromaSaveURL]);
